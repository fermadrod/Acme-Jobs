...
public interface AuditorJobRepository extends AbstractRepository {
	...
	@Query("select a.job from AuditorRecord a where a.auditor.id = ?1 and a.job.status = true")
	Collection<Job> findManyJobByAuditorId(int id);

	@Query("select a.job from AuditorRecord a where not a.auditor.id = ?1 and a.job.status = true")
